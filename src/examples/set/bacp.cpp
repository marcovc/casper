...
void bacp(const Problem<nCourses,nPrereq>& i)
{
	...
	DomVarArray<IntSet> courses(solver,i.nPeriods,range(0,nCourses-1));

	// min/max load per period
	DomVarArray<Int> load(solver,i.nPeriods,i.minLoadPeriod,i.maxLoadPeriod);
	for (Int p = 0; p < i.nPeriods; ++p)
		solver.post( sumEqual(courses[p],credits,load[p]) );

	// min/max courses per period
	DomVarArray<Int> ncourses(solver,i.nPeriods,i.minCoursesPeriod,i.maxCoursesPeriod);
	for (Int p = 0; p < i.nPeriods; ++p)
		solver.post( cardinal(courses[p])==ncourses[p] );

	// channel to period array
	DomVarArray<Int> period(solver,nCourses,0,i.nPeriods);
	for (Int p = 0; p < i.nPeriods; ++p)
