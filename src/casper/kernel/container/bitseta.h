...
class BitsetA : private detail::RevVector<Bool>
{
	public:

//	typedef mybitset<Size>	Super;
	typedef detail::RevVector<Bool> Super;
	...
	typedef	SCIterator<InternalValue,Difference>	scit;
	...
	BitsetA(ICPSolver& solver) : Super(solver,maxSize,false),
							  count(solver,0),mSolver(solver) { }
	BitsetA(ICPSolver& solver, int first, int last) :
							  Super(solver,maxSize,false),
							  count(solver,0),mSolver(solver)
	{ insert(scit(first),++scit(last)); }
	template <class InputIterator>
	BitsetA(ICPSolver& solver,InputIterator first, InputIterator last);
	BitsetA(const BitsetA& s) : Super(s),
								count(s.count),mSolver(s.mSolver) {}
	...
	inline Pair<Iterator, Iterator> equal_range(const Key& k) const;
	inline Iterator lowerBound(const Key& k) const;
	inline Iterator upperBound(const Key& k) const;
	inline Pair<Iterator,bool> insert(ConstReference);
...
template<class T, int maxSize,class AllocT>
template <class InputIterator>
BitsetA<T,maxSize,AllocT>::BitsetA(ICPSolver& solver,
	InputIterator f, InputIterator l) :
	 Super(solver,maxSize,false),count(solver,0),mSolver(solver)
{ insert(f,l); }
...
template<class T, int maxSize,class AllocT>
Pair<typename BitsetA<T,maxSize,AllocT>::Iterator,bool>
...
*/
template<class T, int maxSize,class AllocT>
ostream& operator<<(ostream& o, const BitsetA<T,maxSize,AllocT>& a)
{
	o << "{";
	for (typename BitsetA<T,maxSize,AllocT>::Iterator it = a.begin();
		it != a.end(); ++it)
			o << " " << (int)*it;
#ifdef C_DEBUG_ALL
	o << ",#" << a.count;
