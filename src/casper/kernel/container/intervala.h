...
// TODO: Everything could be improved by storing an explicit end() element
...
struct RInterval
{
	...
	RT	_min;
	RT	_max;
	RInterval(ICPSolver& solver, const T& min, const T& max) :
		_min(solver,min),_max(solver,max) {}
...
using casper::detail::RInterval;

template<class T>
struct less<RInterval<T> > : public binary_function<RInterval<T>,RInterval<T>,bool>
{
//	less(){}
	bool operator()(const RInterval<T>& x, const RInterval<T>& y) const
	{ 	return x.max() < y.min(); 	}
	bool operator()(const RInterval<T>& x, const T& y) const
	{ 	return x.max() < y; 	}
	bool operator()(const T& x, const RInterval<T>& y) const
	{ 	return x < y.min(); 	}
};
};
...
// required forwarded declarations {
template<class T,class Container,class Compare>
class IntervalA;

template<class T,class Container,class Compare>
ostream& operator<<(ostream& os, const IntervalA<T,Container,Compare>& f);
// }
...
*/
template<class T,
         class Container = casper::List,
		 class Compare = std::less<T> >
class IntervalA
{
	// the allocator that will be used to allocate intervals
	typedef RInterval<T>	Interval;
	typedef	typename detail::ContainerTraits<Container,Interval,std::less<Interval> >::Type	DataT;
	...
	IntervalA(ICPSolver&,InputIterator first, InputIterator last);
	IntervalA(ICPSolver&,scit first, scit last);
	template <class InputIterator>
	IntervalA(ICPSolver&,InputIterator first, InputIterator last, const Compare& c);
...
//	IntervalA(key_compare);
	...
	{	return Iterator(data.insert(hint.curInt,Interval(mSolver,f,l)));	}

	template <class InputIterator>
	void insert(InputIterator first, InputIterator last);
	void insert(scit first, scit last);

	friend ostream& operator<< <>(ostream& os, const IntervalA& f);
	...
	insertDI(typename DataT::Iterator& it, ConstReference k)
{
	...
	{
		insertInterval(it,Interval(mSolver,k,k));
		--it;
	}
	else
	if (k < it->min())
...
Void IntervalA<T,Container,Compare>::initListAndSentinel()
{
	...
	NodeI* cur = new (solver().heap())
		Node(Interval(mSolver,0,0),RPNodeI(mSolver),RPNodeI(mSolver));
...
*/
template<class T, class Container, class Compare>
IntervalA<T,Container,Compare>::IntervalA(ICPSolver& mSolver) :
	mSolver(mSolver),data(mSolver)
{ 	initListAndSentinel();	}
	...
	erase(typename IntervalA<T,Container,Compare>::Iterator p)
{
	...
	// if erasing interior element
	{
		...
		insertInterval(p.curInt,Interval(mSolver,tmp,p.curVal - 1));
	}
}
...
*/
template<class T, class Container, class Compare>
Pair<typename IntervalA<T,Container,Compare>::Iterator,
	 typename IntervalA<T,Container,Compare>::Iterator>
	 IntervalA<T,Container,Compare>::
	equal_range(const key_type& k) const
{
	Pair<Iterator,Iterator> ret(Iterator(*this),Iterator(*this));
...
*/
template<class T, class Container, class Compare>
Pair<typename IntervalA<T,Container,Compare>::Iterator,bool>
	IntervalA<T,Container,Compare>::
	insert(ConstReference k)
{
	Pair<Iterator,bool> ret(Iterator(),true);
	ret.first.curVal = k;
	ret.first.curInt = data.lowerBound(Interval(mSolver,(k==limits<T>::min())?k:k-1,k));
...
template <class InputIterator>
IntervalA<T,Container,Compare>::
	IntervalA(ICPSolver& mSolver,InputIterator first, InputIterator last) :
		mSolver(mSolver),
		data(mSolver)
{
	initListAndSentinel();
...
*/
template<class T, class Container, class Compare>
IntervalA<T,Container,Compare>::IntervalA(ICPSolver& mSolver,scit amin, scit amax) :
		mSolver(mSolver),data(mSolver)
{
	initListAndSentinel();
	insertInterval(Interval(mSolver,*amin,*--amax));
}
...
IntervalA<T,Container,Compare>::
	IntervalA(ICPSolver& mSolver,InputIterator first, InputIterator last,
			  const Compare& c) : mSolver(mSolver),data(mSolver)
{
	initListAndSentinel();
	insert(first,last);
}
	...
	insert(Iterator hint, ConstReference k)
{
	...
	else	// hint is wrong, insert with no hint
	{
		if (hint.curInt != data.begin())
			++hint.curInt;
		Interval tmp(mSolver,(k==limits<T>::min())?k:k-1,k);
...
{
	--amax;
	Interval tmp(Reversible<T>(mSolver,(*amin==limits<T>::min())?(*amin):(*amin-1)),
			 Reversible<T>(mSolver,*amin));
	typename DataT::Iterator it = data.lowerBound(tmp);
	if (it == data.end() || (it->min() > limits<T>::min() && *amax < it->min() - 1))
		insertInterval(it,Interval(Reversible<T>(mSolver,*amin),
								Reversible<T>(mSolver,*amax)));
	else
	{
		if (*amin < it->min())
			it->min() = *amin;
