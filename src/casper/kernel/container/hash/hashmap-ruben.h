						 ...
						 HashMapHasher<Pair<const KeyT,DataT>,Hasher> >
{
	...
	/// TO BE REMOVED
	Iterator find(const Key& key, const Data& data) const;
...
Bool HashMap<KeyT,DataT,EqualKey,Hasher>::insert(const KeyT& key, const DataT& data)
{	return Super::insert(Value(key,data));	}

template<class KeyT, class DataT, class EqualKey, class Hasher>
typename HashMap<KeyT,DataT,EqualKey,Hasher>::Iterator
HashMap<KeyT,DataT,EqualKey,Hasher>::find(const KeyT& key) const
{	return Super::find(Value(key,Data()));	}

template<class KeyT, class DataT, class EqualKey, class Hasher>
typename HashMap<KeyT,DataT,EqualKey,Hasher>::Iterator
HashMap<KeyT,DataT,EqualKey,Hasher>::find(const KeyT& key, const DataT& data) const
{	return Super::find(Value(key,data));	}

template<class KeyT, class DataT, class EqualKey, class Hasher>
Bool HashMap<KeyT,DataT,EqualKey,Hasher>::erase(const Key& key)
{	return Super::erase(Value(key,Data()));	}

template<class KeyT, class DataT, class EqualKey, class Hasher>
DataT& HashMap<KeyT,DataT,EqualKey,Hasher>::operator[](const KeyT& key)
{
...
template<class KeyT, class DataT, class EqualKey, class Hasher>
const DataT& HashMap<KeyT,DataT,EqualKey,Hasher>::operator[](const KeyT& key) const
{
	Iterator it = find(key);
	assert(it != this->end());
	return it->second;
