...
struct IMutFilter
{
	virtual Filter instantiate(ICPSolver&) const = 0;
	virtual ~IMutFilter() {}
};
...
struct MutFilterView : IMutFilter
{
	...
	Filter instantiate(ICPSolver& s) const
	{	return Bnd(pv); /*new (s.heap()) BndFilterView<View>(s,pv.getCRef())*/	}
	View	pv;
};
 ...
 *  \sa IMutFilter
 */
struct MutFilter : detail::PImplIdiom<IMutFilter>
{
	typedef MutFilter						Self;
	typedef detail::PImplIdiom<IMutFilter> 	Super;
	...
	MutFilter(const View& v) :
		Super(new (getCPSolver(v).heap())
			MutFilterView<View>(v)) {}

	Filter instantiate(ICPSolver& solver) const
