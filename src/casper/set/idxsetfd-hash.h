...
struct IdxSetFD
{
	...
	private:
		...
		struct	Elem 
		{
			...
			Elem(ICPSolver& solver, const Value& value) : 
				value(value),state(solver,poss) 
				{}
			Elem(ICPSolver& solver, const Value& value, State state) : 
				value(value),state(solver,state) 
				{}
			Value				value;
	...
	public:
		...
		IIterator findInLUB(const Value& v) const;
	...
	private:
		...
		Void onBeforeInsert(ElemsIterator b,ElemsIterator e);
		Void onBeforeInsert(ElemsIterator e);
		Void onBeforeErase(ElemsIterator b,ElemsIterator e);
...
template<class T>
typename IdxSetFD<T>::IIterator IdxSetFD<T>::findInLUB(const Value& v) const 
{
	typename Index::ConstIterator iit = index.find(v);
	if (iit == index.end())
		return elems.end();
...
template<class T>
Void IdxSetFD<T>::onBeforeInsert(ElemsIterator b,ElemsIterator e)
{	
#ifdef CASPER_OLDDELTAS
...
#endif	
/*	cninserts++;
	for (ElemsIterator aux = b; aux != e; ++aux)
		++cavginsertsize;*/
}
...
template<class T>
Void IdxSetFD<T>::onBeforeErase(ElemsIterator b,ElemsIterator e)
{	
#ifdef CASPER_OLDDELTAS	
	mDelta.b = b;	mDelta.e = e;	
#else	
...
{
	for (ElemsIterator itb = p; itb != q;  )
		if (itb->state == Elem::poss)
		{
			ElemsIterator ite = itb;
			UInt nremoves = 0;
			while (ite != q and ite->state == Elem::poss)
			{	
				ite->state = Elem::out;
				++ite;
				++nremoves;
 ...
 **/
template<class T>
Bool IdxSetFD<T>::insertBefore(IIterator a, PIterator p)
{
	onBeforeUpdate(p);
	inSet.insert(a,*p);
	++inSetSize;
 ...
 **/
template<class T>
Bool IdxSetFD<T>::insertBefore(IIterator a, PIterator p, PIterator q)
{
	assert(p != q);	
	onBeforeUpdate(p,q);
	UInt c = count(p,q);
...
{	
	if (++InputIterator(b) == e)	// saves time when inserting a single element
		return insert(*b);

	while (b != e)
	{	
		typename Index::Iterator iit = index.find(*b);
		if (iit == index.end() or iit->second->state == Elem::out)
			return false;
		if (iit->second->state == Elem::poss)
		{
			UInt nremoves = 0;
			ElemsIterator itb = iit->second;
			ElemsIterator ite = itb;
			while (b != e and ite != elems.end() and
				   detail::TestRel<T>::lessEqual(*b,ite->value))
			{	
				if (not detail::TestRel<T>::less(*b,ite->value) and
					ite->state == Elem::poss)
				{
					ite->state = Elem::out;
					++ite;
...
{	
	if (++InputIterator(b) == e)	// saves time when erasing a single element
		return erase(*b);
