...
#include <casper/kernel/container/rslist.h>
...
struct Edge
{
	...
	Edge(ICPSolver& s,PBoy pBoy, PGirl pGirl) :
...
struct DiGraphNode
{
	...
	DiGraphNode(ICPSolver& s, UInt idx) :
...
struct DiGraphBoyNode : DiGraphNode
{
	...
	DiGraphBoyNode(ICPSolver& s, UInt idx) :
		Super(s,idx) {}
...
struct DiGraphGirlNode : DiGraphNode
{
	...
	DiGraphGirlNode(ICPSolver& s, UInt idx) :
		Super(s,idx) {}
...
struct BiDiGraph
{
	...
	typedef Vector<PBoyNode>	BoyNodes;
	typedef BoyNodes::Iterator	BoyNodeIterator;

	typedef Vector<PGirlNode>	GirlNodes;
	typedef GirlNodes::Iterator	GirlNodeIterator;

	BiDiGraph(ICPSolver& solver, UInt nBoys, UInt nGirls) :
		solver(solver),
	...
	{
		PEdge e = new (solver.heap()) Edge(solver,b,g);
		setGirlBoyEdge(e);
	}
...
struct MaximalMatching
{
	...
	MaximalMatching(ICPSolver& sol,BiDiGraph& net) :
							net(net) {}
...
struct AllDiffHelper
{
	...
	AllDiffHelper(ICPSolver& sol,BiDiGraph& net) :
			net(net),
			d(sol.heap(),net.nBoys(),net.nGirls(),0),
			toVisit(sol.heap(),net.nBoys()+net.nGirls(),0),
			s(sol.heap(),net.nBoys()*net.nGirls(),0),
			rindex(sol.heap(),net.nNodes(),0),
			pre(sol.heap(),net.nNodes(),0),
			low(sol.heap(),net.nNodes(),0),
			cnt1(0),
			cnt0(0)
		{}
	...
	BiDiGraph&		net;
	Array<Int,2> 	d;			//  where to mark directed paths and
								//  strongly connected components
	Vector<PBoyNode> 	toVisit;	//	aux vector used in markDirectPaths
	Vector<PNode> 	s;			// 	global stack for scc
	Int 			sSize;		// 	stack size (for efficiency)
	UInt			c;			//  aux counter for SCC
	UInt			index;		//  aux counter for SCC
	Vector<Int> 	rindex;		//  aux array to color SCC
