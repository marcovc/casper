...
class VarValGraph
{
    ...
    unsigned int cnt0;
    unsigned int cnt1;
    detail::Stack<Node<View>*> n_s;

    typedef typename View::Elem	ViewDom;
  	typedef View	DomArray;
  	public:
    VarValGraph(ICPSolver& s,int, View&, const int*, int, unsigned int);
    VarValGraph(ICPSolver& s,int, View&, int, unsigned int, int);
    ~VarValGraph(void);

    size_t size;

    static VarValGraph* init(ICPSolver&,int,View&);
...
template <class View>
forceinline
VarValGraph<View>::VarValGraph(ICPSolver& s,int n_var0, View& x,
				    int n_val0,
				    unsigned int n_edges, int min)
    :  solver(s),n_var(n_var0), n_val(n_val0),
       count(0), cnt0(0), cnt1(0), n_s(s.heap()/*,n_var+n_val*/)
{
    ...
    mem = reinterpret_cast<char*>(solver.heap().allocate(size));
...
template <class View>
VarValGraph<View>*
VarValGraph<View>::init(ICPSolver& s,int n_var, View& x)
{
    ...
    //if (static_cast<unsigned int>(n_var)*2 >= width)
      return new (s.heap()) VarValGraph<View>(s,n_var,x,static_cast<int>(width),size,min);
}
...
{
	// Stack for var nodes to be rematched
	Array<VarNode<View>*>	re(solver.heap(),n_var);
    int n_re = 0;
    // Synchronize nodes
    for (int i = n_var; i--; )
    {
