...
struct NoElementView
{
	NoElementView(const Obj& r)
	{
		std::ostringstream os;
		os << r;
		throw Casper::Exception::UndefinedView(os.str().c_str(),"ElementView");
	}
	const Elem& 	get(uint) const 	{	throw 0; }
};
...
struct ElementView
{
	...
	const Elem& 	get(uint idx) const 	{	return v[idx];	}
	const Obj& v;
};
...
// for desambiguation between the two above definitions
template<class SeqT,class Index>
struct ElementView<Rel2<Element,SeqT,Index>,typename Traits::GetTermElem<Rel2<Element,SeqT,Index> >::Type>
{
	typedef typename Traits::GetTermElem<Rel2<Element,SeqT,Index> >::Type Elem;
	typedef Rel2<Element,SeqT,Index> RelT;
	ElementView(const RelT& v) : v(v.p1),i(getState(v),v.p2) {}
	const Elem& get(uint idx) const
	{
