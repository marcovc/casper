...
#ifndef CASPER_LP_LINEAREXPRVALVIEW_H_
#define CASPER_LP_LINEAREXPRVALVIEW_H_
...
struct LinearExprValView
{
	typedef typename Casper::Traits::GetEval<T>::Type	Eval;
	LinearExprValView(Store& store, const T& v) : v(store,v) {}
...
struct LinearExprValView<Store,double>
{
	LinearExprValView(Store& store, const double& v) : v(v) {}
...
template<class Store>
struct LinearExprValView<Store,Var>
{
	LinearExprValView(Store& store, const Var& v) : v(v) {}
	bool isLinear() const {	return true; }
...
struct LinearExprValView<Store,Rel2<Add,T1,T2> >
{
	LinearExprValView(Store& store, const Rel2<Add,T1,T2>& r) :
		v1(store,r.p1),
		v2(store,r.p2) {}
...
template<class Store,class T1,class T2>
struct LinearExprValView<Store,Rel2<Sub,T1,T2> >
{
	LinearExprValView(Store& store, const Rel2<Sub,T1,T2>& r) :
		v1(store,r.p1),
		v2(store,r.p2) {}
	bool isLinear() const {	return v1.isLinear() and v2.isLinear(); }
	bool isConstant() const {	return v1.isConstant() and v2.isConstant(); }
...
template<class Store,class T1,class T2>
struct LinearExprValView<Store,Rel2<Mul,T1,T2> >
{
	LinearExprValView(Store& store, const Rel2<Mul,T1,T2>& r) :
		v1(store,r.p1),
		v2(store,r.p2) {}
	bool isLinear() const
	{
...
template<class Store,class T1,class T2>
struct LinearExprValView<Store,Rel2<Div,T1,T2> >
{
	LinearExprValView(Store& store, const Rel2<Div,T1,T2>& r) :
		v1(store,r.p1),
		v2(store,r.p2) {}
	bool isLinear() const
	{
...
{
	LinearExprValView(Store& store, const Rel2<Element,VarArray,T2>& r) :
		v2(store,r.p2),
		views(store,r.p1.size())
	{
		for (uint i = 0; i < views.size(); ++i)
			views[i] = new (store) LinearExprValView<Store,Var>(store,r.p1[i]);
	}
	bool isLinear() const
	{
	...
	Util::StdVector<LinearExprValView<Store,Var>*> views;
};
...
#endif /* CASPER_LP_LINEAREXPRVALVIEW_H_ */
